...
def get_database_url():
    ...
    if "sslmode=" not in database_url:
        ...
        sslmode = "disable" if parsed_url.hostname in local_hosts else "require"
...
- Treat the Flight Results section as the source of truth for flights.
- Do not rewrite, soften, or replace the flight details with a generic disclaimer.
- If flight pricing is unavailable, keep that note only in the flight section and still build the rest of the itinerary normally.
...
Flight handling rules:
- Preserve the flight details exactly as provided in Flight Results.
- Do not replace them with a generic "technical issues" message unless Flight Results already says that.
- If pricing is unavailable, mention that only once in the Flight Information section.
...
- Do not repeat the same flight disclaimer in multiple sections.
